...
class ClassifierTester:
    ...
    def __init__(self, config_path: str = "config.yaml", model_choice: str = "bioclip2") -> None:
        ...
        self.bioinfo_file = Path(__file__).resolve().parent / "species_pipeline" / "outputs" / "04_obis_prior_db.json"
    ...
    def _load_bioinfo(self):
        ...
        # Handle obis_prior_db format which contains an 'items' list
        ...
        for item in data:
            ...
            species_full = self._extract_species_name(item)
            comm_kor_nm = self._extract_common_name(item)
    ...
    @staticmethod
    def _extract_species_name(item: dict) -> str:
        for key in ("SpcScitfNm", "학명", "?숇챸"):
            value = item.get(key)
    ...
    @staticmethod
    def _extract_common_name(item: dict) -> str:
        for key in ("CommKorNm", "국명", "援?챸"):
            value = item.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
        return ""
